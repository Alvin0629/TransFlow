...
class Decoder(nn.Module):
    ...
    def forward(self, cost_memory, context, Occ_map, data={}, flow_init=None):
        cost_maps = data['cost_maps']
        coords0, coords1 = initialize_flow(context)

        N, fC, fH, fW = context.shape
        corrMap = Occ_map.squeeze().view(N, fH*fW, -1)
        Match_Corrmap = F.softmax(corrMap, dim=2) * F.softmax(corrMap, dim=1) # (N, fH*fW, fH*fW)

        if flow_init is not None:
            coords1 = coords1 + flow_init
            
        else:
            (match12, match_idx12) = Match_Corrmap.max(dim=2)
            (match21, match_idx21) = Match_Corrmap.max(dim=1)
            ...
            coords_index = torch.arange(fH * fW).unsqueeze(0).repeat(N,
                    1).to(Match_Corrmap.device)
        ...
        for itr in range(self.depth):
            ...
            cost_forward = self.encode_flow(cost_maps, coords1)
        ...
        if self.training:
            return flow_predictions, Match_Corrmap
